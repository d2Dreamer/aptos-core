...
struct MockSnapshotStore<K: TestKey, V: TestValue> {
    ...
    usage_store: RwLock<HashMap<Version, (usize, usize)>>,
}
    ...
    V: TestValue,
{
    ...
    fn get_stored_usage(&self, version: Version) -> (usize, usize) {
        *self
            .usage_store
            .read()
            .get(&version)
    ...
    fn calculate_usage(&self, version: Version) -> (usize, usize) {
        let mut items = 0;
        let mut bytes = 0;
        for ((k, ver), v) in self.kv_store.read().iter() {
            if *ver == version {
                items += 1;
                bytes += k.key_size() + v.value_size();
            }
        }
        (items, bytes)
    }
    ...
    V: TestValue,
{
    ...
    fn write_usage(&self, version: Version, items: usize, total_bytes: usize) -> Result<()> {
        self.usage_store
            .write()
            .insert(version, (items, total_bytes));
        Ok(())
    }
}
    ...
    V: crate::TestKey + crate::TestValue,
{
    ...
    assert_eq!(usage_stored.0, tree.get_leaf_count(version).unwrap());
}
